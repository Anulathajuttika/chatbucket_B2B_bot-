...
_embeddings = OpenAIEmbeddings(model=config.EMBEDDING_MODEL, api_key=config.OPEN_AI_API_KEY)
...
def retrieve_context(query: str, k: int = config.TOP_K) -> list[str]:
    try:
        ...
        relevant = [(doc, score) for doc, score in docs_with_scores if score >= config.MIN_RELEVANCE_SCORE]

        print("\n========== RETRIEVED CHUNKS ==========")
        for i, (doc, score) in enumerate(relevant, 1):
            print(f"\n----- Chunk {i} (score={score:.3f}) -----")
            print(doc.page_content)
        print(f"====== {len(relevant)}/{len(docs_with_scores)} chunks passed "

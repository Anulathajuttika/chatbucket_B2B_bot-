...
KNOWLEDGE_FILE = os.path.join(BASE_DIR, "knowledge_b2b.md")
COLLECTION_NAME = "chatbucket_b2b_knowledge"
VECTOR_DB_DIR = os.path.join(BASE_DIR, "vector_db")
LOGS_DIR = os.path.join(BASE_DIR, "logs")

# --- Retrieval ---
# TOP_K = 4 used to starve "enumerate everything" questions (e.g. "tell all
# features of the todozee") of context: ToDoZee's full feature list only lives
# in knowledge.md as ~7-8 separate ~700-800-char chunks (one per category,
# e.g. "AI Tools", "Spiritual", "Kitchen Zone") plus the ToDoZee doc's own
# "Quick feature index" table chunk(s) -- no single chunk, or even the top 4,
# contains anywhere near the full 29/30-item list. Measured empirically
# against this knowledge base: retrieving k=20 candidates (then still
# filtering by MIN_RELEVANCE_SCORE below) is what it actually takes for every
# category chunk + the quick-index chunk to be in the candidate pool at once;
# k=25/30 retrieved nothing further, so 20 is not an arbitrary/oversized
# guess. Raising k this much is safe on its own because MIN_RELEVANCE_SCORE
# still filters every candidate by score regardless of k (verified: greetings/
# small-talk/off-topic/prompt-injection queries came back with ZERO chunks
# above the cutoff even at k=20 -- see MIN_RELEVANCE_SCORE note below).
# --- Retrieval ---
TOP_K = 8  # more chunks = more complete context for broad/comparative questions
